...
exports.DEFAULT_RC_FILENAME = fslib_2.toFilename(`.yarnrc.yml`);
exports.DEFAULT_LOCK_FILENAME = fslib_2.toFilename(`yarn.lock`);
exports.SECRET = `********`;
var SettingsType;
...
exports.formatColors = chalkOptions.level >= 3 ? new Map([
...
exports.coreDefinitions = {
    ...
    compressionLevel: {
        ...
        default: fslib_1.DEFAULT_COMPRESSION_LEVEL,
    },
    virtualFolder: {
    ...
    const interpretValue = () => {
        ...
        switch (definition.type) {
            case SettingsType.ABSOLUTE_PATH:
                return fslib_2.ppath.resolve(folder, fslib_2.npath.toPortablePath(valueWithReplacedVariables));
        ...
        case SettingsType.ABSOLUTE_PATH:
            {
                ...
                if (configuration.projectCwd === null) {
                    if (fslib_2.ppath.isAbsolute(definition.default)) {
                        return fslib_2.ppath.normalize(definition.default);
                    }
                    ...
                    if (Array.isArray(definition.default)) {
                        return definition.default.map((entry) => fslib_2.ppath.resolve(configuration.projectCwd, entry));
                    }
                    else {
                        return fslib_2.ppath.resolve(configuration.projectCwd, definition.default);
                    }
                }
            }
            break;
...
function transformConfiguration(rawValue, definition, transforms) {
    ...
    if (definition.type === SettingsType.ABSOLUTE_PATH && typeof rawValue === `string` && transforms.getNativePaths)
        return fslib_2.npath.fromPortablePath(rawValue);
...
class Configuration {
    ...
    static async find(startingCwd, pluginConfiguration, { lookup = ProjectLookup.LOCKFILE, strict = true, usePath = false, useRc = true } = {}) {
        ...
        switch (lookup) {
            ...
            case ProjectLookup.NONE:
                {
                    if (fslib_2.xfs.existsSync(fslib_2.ppath.join(startingCwd, `package.json`))) {
                        projectCwd = fslib_2.ppath.resolve(startingCwd);
                    }
                    else {
                        projectCwd = null;
                    }
        ...
        if (pluginConfiguration !== null) {
            ...
            const importPlugin = (pluginPath, source) => {
                const { factory, name } = nodeUtils.dynamicRequire(fslib_2.npath.fromPortablePath(pluginPath));
            ...
            if (environmentSettings.plugins) {
                for (const userProvidedPath of environmentSettings.plugins.split(`;`)) {
                    const pluginPath = fslib_2.ppath.resolve(startingCwd, fslib_2.npath.toPortablePath(userProvidedPath));
            ...
            for (const { path, cwd, data } of rcFiles) {
                ...
                for (const userPluginEntry of data.plugins) {
                    const userProvidedPath = typeof userPluginEntry !== `string`
                        ...
                        : userPluginEntry;
                    const pluginPath = fslib_2.ppath.resolve(cwd, fslib_2.npath.toPortablePath(userProvidedPath));
    ...
    static async findRcFiles(startingCwd) {
        ...
        while (nextCwd !== currentCwd) {
            currentCwd = nextCwd;
            const rcPath = fslib_2.ppath.join(currentCwd, rcFilename);
            if (fslib_2.xfs.existsSync(rcPath)) {
                const content = await fslib_2.xfs.readFilePromise(rcPath, `utf8`);
                let data;
                try {
    ...
    static async findHomeRcFile() {
        ...
        const homeRcFilePath = fslib_2.ppath.join(homeFolder, rcFilename);
        if (fslib_2.xfs.existsSync(homeRcFilePath)) {
            const content = await fslib_2.xfs.readFilePromise(homeRcFilePath, `utf8`);
    ...
    static async findProjectCwd(startingCwd, lockfileFilename) {
        ...
        while (nextCwd !== currentCwd) {
            currentCwd = nextCwd;
            if (fslib_2.xfs.existsSync(fslib_2.ppath.join(currentCwd, fslib_2.toFilename(`package.json`))))
                projectCwd = currentCwd;
            if (lockfileFilename !== null) {
                if (fslib_2.xfs.existsSync(fslib_2.ppath.join(currentCwd, lockfileFilename))) {
                    projectCwd = currentCwd;
                    break;
                }
            }
    ...
    static async updateConfiguration(cwd, patch) {
        const rcFilename = getRcFilename();
        const configurationPath = fslib_2.ppath.join(cwd, rcFilename);
        const current = fslib_2.xfs.existsSync(configurationPath)
            ? parsers_1.parseSyml(await fslib_2.xfs.readFilePromise(configurationPath, `utf8`))
            : {};
        let patched = false;
        ...
        if (!patched)
            return;
        await fslib_2.xfs.changeFilePromise(configurationPath, parsers_1.stringifySyml(current), {
    ...
    getSubprocessStreams(logFile, { header, prefix, report }) {
        let stdout;
        let stderr;
        const logStream = fslib_2.xfs.createWriteStream(logFile);
    ...
    format(text, colorRequest) {
        if (colorRequest === FormatType.PATH)
            text = fslib_2.npath.fromPortablePath(text);
        if (!this.get(`enableColors`))
            return text;
        let color = exports.formatColors.get(colorRequest);
