...
const legacy_format_issue_1 = require("../test/formatters/legacy-format-issue");
...
function generatePrompt(vulns, policy, prefix, options) {
    ...
    let prompts = vulns.map((vuln, i) => {
        ...
        if (group) {
            ...
            messageIntro = util_1.format('✗ %s %s %s introduced %s %s', group.count, `${severity}${originalSeverityStr}`, issues, joiningText, group.affected.full);
            messageIntro = createSeverityBasedIssueHeading(messageIntro, vuln.severity);
        }
        else {
            infoLink += chalk_1.default.underline('/vuln/' + vuln.id);
            messageIntro = util_1.format('✗ %s severity %s found in %s, introduced via', `${severity}${originalSeverityStr}`, vuln.type === 'license' ? 'issue' : 'vuln', vulnIn, from);
            messageIntro = createSeverityBasedIssueHeading(messageIntro, vuln.severity);
